...
const INTEGRATION_UI = {
  WEBSITE_API: {
    icon: Link2,
    fields: [
      ["websiteName", "Website Name", "Student Mentor Website"],
      ["allowedOrigin", "Allowed Origin", "https://studentmentor.co.in"],
      ["defaultSourceKey", "Default Lead Source Key", "WEBSITE_FORM"],
    ],
  },
  ...
  GOOGLE_FORMS: {
    icon: FileCheck,
    fields: [
      ["formName", "Form Name", "Counselling Registration Form"],
      ["formId", "Google Form ID", "Form ID"],
      ["defaultSourceKey", "Default Lead Source Key", "GOOGLE_FORM"],
    ],
  },
  ...
  META_LEADS: {
    icon: Facebook,
    fields: [
      ["pageName", "Page Name", "Student Mentor"],
      ["pageId", "Meta Page ID", "Page ID"],
      ["formId", "Lead Form ID", "Form ID"],
      ["defaultSourceKey", "Default Lead Source Key", "DM_LEADS"],
    ],
  },
  ...
  WHATSAPP: {
    icon: MessageCircle,
    fields: [
      ["businessName", "Business Name", "Student Mentor"],
      ["phoneNumber", "WhatsApp Number", "+91..."],
      ["businessAccountId", "Business Account ID", "WABA ID"],
    ],
  },
  ...
  EMAIL_SMTP: {
    icon: AtSign,
    fields: [
      ["senderName", "Sender Name", "Student Mentor"],
      ["senderEmail", "Sender Email", "hello@company.com"],
  ...
  GENERIC_WEBHOOK: {
    icon: Webhook,
    fields: [
      ["integrationName", "Integration Name", "External CRM"],
      ["externalSystem", "External System", "System name"],
      ["defaultSourceKey", "Default Lead Source Key", "OTHER"],
    ],
  },
};
...
export default function SettingsView({
  ...
  async function saveIntegration() {
    ...
    try {
      const data =
        await apiRequest(
          `/api/client/integrations/${editingIntegration.provider}`,
          {
            method: "PUT",
            body:
              JSON.stringify({
  ...
  async function toggleIntegration(
    ...
    try {
      const data =
        await apiRequest(
          `/api/client/integrations/${integration.provider}/enabled`,
          {
            method: "PATCH",
            body:
              JSON.stringify({
  ...
  return (
    <div className="space-y-4">
                                ...
                                <div className="text-[10px] text-slate-400">
                                  {configured
                                    ? `${Object.keys(
                                        integration.config ||
                                          {}
                                      ).length} configuration field(s) saved`
                                    : "Not configured yet"}
                                </div>
      ...
      {integrationModalOpen &&
        editingIntegration && (
        <div className="fixed inset-0 z-[140] bg-slate-950/55 backdrop-blur-[2px] flex items-center justify-center p-4">
          <div className="w-full max-w-xl max-h-[90vh] overflow-y-auto bg-white border border-slate-200 rounded-2xl shadow-2xl">
            <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
              <div>
                <div className="text-base font-bold text-slate-950">
